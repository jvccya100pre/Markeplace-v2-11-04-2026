...
class CatalogController
{
    ...
    {
        $products = db()->query('SELECT name, internal_code, price_retail, price_wholesale FROM ' . table_name('products') . ' WHERE price_retail > 0 ORDER BY id DESC LIMIT 300')->fetchAll();
        $date = date('Ymd_His');
        $filename = 'catalogo_usd_' . $date . '.pdf';

        $lines = array();
        $lines[] = 'www.tutiendaonlinelq.com';
        $lines[] = 'WhatsApp: +58 123 456 7890'; // Placeholder, need to get from settings
        $lines[] = 'Catalogo en USD';
        $lines[] = 'Fecha: ' . date('d/m/Y H:i');
        $lines[] = '----------------------------------------';
        foreach ($products as $p) {
            $usdRetail = $p['price_retail'] / get_exchange_rate();
            $usdWholesale = $p['price_wholesale'] > 0 ? $p['price_wholesale'] / get_exchange_rate() : 0;
            $lines[] = 'Foto: [imagen] | Codigo: ' . $p['internal_code'] . ' | Detal: $' . number_format($usdRetail, 2) . ' | Mayor: $' . number_format($usdWholesale, 2);
        }
